...
sample_point<-function(imgLR,n_points=1000){
...
extract_center<-function(data_index){
  center_data<-expandRows(data_index, "freq2")
  center_value<-mapply(value_img_LR,center_data$samp_row,center_data$samp_col)
  return(center_value)
}

```

###extract value from low resolution dataset function
```{r}
...
extract_value_LR<-function(image_index){
  ...
  neighbor_list<-mapply(extract_eight,data_index$samp_row, data_index$samp_col)
  df_neighbor <- data.frame(do.call(rbind,neighbor_list))
  center_data<-expandRows(data_index, "freq1")
  #value of center pixel
  center_value<-mapply(value_img_LR,center_data$samp_row,center_data$samp_col)
  #add black around the LR image
  addblack_img<-addblack(imgLR)
...
extract_value_HR<-function(image_index){
  ...
  #subtract the value of center from the value of neighbor 8 pixels
  value_HR<-subpixel_value-center_HR
  return(value_HR)
}
```
```{r}
#imgLR1<-readImage("~/Desktop/5243project3/train_set/LR/img_0001.jpg")
...
#extract_value<-function(index){
  ...
  #addblack_img<-addblack(imgHR)
  #center_HR<-extract_center(data_index)
  #HR<-extract_value_HR(index)
  #return(list(data_index,LR,HR))
#}
#a<-mapply(extract_value,1)
```
